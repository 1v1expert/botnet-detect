                ...
                for ii, iconcept in enumerate(self.concepts):
                    for ini in range(iconcept.numOfTerms):
                        self.layerf[3][oj][omj][ii][ini] = self.layerx[2][ii][ini] * \
                                                           (1 - subsethood(iconcept.C[ini], iconcept.sigma[ini],
                                                                           oconcept.C[omj], oconcept.sigma[omj]))
        for oj, oconcept in enumerate(self.concepts):
            for omj in range(oconcept.numOfTerms):
                numerator = 0
                denominator = 0
                            ...
                            for ti, tconcept in enumerate(self.concepts):
                                if ti != l:
                                    for tni in range(tconcept.numOfTerms):
                                        numerator += self.layerf[3][l][ml][ti][tni] * tconcept.C[tni] * tconcept.sigma[
                                            tni]
                                        denominator += self.layerf[3][l][ml][ti][tni] * tconcept.sigma[tni]
                            delta_yl = lconcept.xi[ml]
                            delta_x = (iconcept.C[ini] * iconcept.sigma[ini] * denominator - iconcept.sigma[
                                ini] * numerator) / denominator ** 2
                            delta_f_C = self.layerx[2][ii][ini] * (
                                        2 * (self.layerx[1][ii] - iconcept.C[ini]) / iconcept.sigma[ini] ** 2) \
                                        * (1 - subsethood(iconcept.C[ini], iconcept.sigma[ini], lconcept.C[ml],
                                                          lconcept.sigma[ml])) \
                                        - self.layerx[2][ii][ini] * deltaSubsetC(iconcept.C[ini], iconcept.sigma[ini],
                                                                                 lconcept.C[ml], lconcept.sigma[ml])
                            delta_f_Sigma = self.layerx[2][ii][ini] * (
                                        2 * (self.layerx[1][ii] - iconcept.C[ini]) ** 2 / iconcept.sigma[ini] ** 3) \
                                            * (1 - subsethood(iconcept.C[ini], iconcept.sigma[ini], lconcept.C[ml],
                                                              lconcept.sigma[ml])) \
                                            - self.layerx[2][ii][ini] * deltaSubsetSigma(iconcept.C[ini],
                                                                                         iconcept.sigma[ini],
                                                                                         lconcept.C[ml],
                                                                                         lconcept.sigma[ml])
